...
def test_D():
    ...
    x1, y1 = np.random.uniform(
        0.0, 2 * np.pi), np.random.uniform(0.0, 2 * np.pi)
    # test 2->2 function
    def f1(x, y): return [np.cos(x + y), np.sin(x - y)]
    def df1_x(x, y): return np.array([-np.sin(x + y), np.cos(x - y)])
    def df1_y(x, y): return np.array([-np.sin(x + y), -np.cos(x - y)])
...
def test_dlambda_dtheta():
    ...
    assert np.abs(utils.dlambda_dtheta(theta) -
                  utils.D(utils.theta_to_lambda, 0, args=[theta])) < 1e-6

def test_vectorisation():
    theta = np.random.uniform(0, np.pi / 2, 100)

    vec_lambda = utils.theta_to_lambda(theta)
    for i in range(len(vec_lambda)):
        assert abs(vec_lambda[i] - utils.theta_to_lambda(theta[i])) < 1e-8

    vec_dlambda = utils.dlambda_dtheta(theta)
    for i in range(len(vec_dlambda)):
        assert abs(vec_dlambda[i] - utils.dlambda_dtheta(theta[i])) < 1e-8